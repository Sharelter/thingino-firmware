#!/bin/bash
#
# Embedded Linux boot partition extractor
#
# This script extracts the boot partition from a firmware dump.
...
for p in ${mtdparts//,/ }; do
    ...
    if [ "$p_name" = "boot" ]; then
        echo_c 190 "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
        boot_file=boot.bin
        boot_offset=$offset_bytes
        boot_size=$p_size_bytes
    fi
...
[ -z "$boot_file" ] && die "Boot partition not found!"

say "extract boot partition from full dump: $boot_size bytes at offset $boot_offset"
run "dd if=$full_binary_file bs=1 skip=$boot_offset count=$boot_size of=$boot_file status=progress"

say "Boot partition extracted to $boot_file"
